...
def load_rag_chain():
    ...
    # --- HATA DÜZELTMESİ (NoneType Hatası) ---
    # Veri setinde 'None' (boş) değerler olabilir. Bunları filtreliyoruz.
    st.write("Veri seti temizleniyor...")
    reviews_list = dataset['Movie_Info'][:100]
    filtered_reviews = [review for review in reviews_list if review is not None and isinstance(review, str)]
    bolum_metni = " ".join(filtered_reviews)
    # --- DÜZELTME SONA ERDİ ---
    ...
    # 4. Vektör Veritabanını Oluştur (FAISS)
